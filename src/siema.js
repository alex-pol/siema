...
export default class Siema {
  ...
  buildSliderFrame() {
    this.widthItem =
      (this.selectorWidth / this.perPage) * (1 - this.originalOffset);
  ...
  resolveSlidesNumber() {
    ...
    if (typeof this.config.offset === "number") {
      ...
      this.innerOffset = this.config.center
        ? (this.originalOffset / 2) * this.perPage
        : this.originalOffset;
      return;
    }
    ...
    if (typeof this.config.offset === "object") {
      for (const viewport in this.config.offset) {
        if (window.innerWidth >= viewport) {
          this.originalOffset =
            this.config.offset[viewport] === 1
              ? 0
              : this.config.offset[viewport];
          this.innerOffset = this.config.center
            ? (this.originalOffset / 2) * this.perPage
            : this.originalOffset;
        }
  ...
  updateAfterDrag() {
    ...
    const howManySliderToSlide = this.config.multipleDrag
      ? Math.ceil(movementDistance / this.widthItem)
      : 1;
  ...
  getCurrentOffset() {
    ...
    return this.config.center
      ? currentSlide * this.widthItem -
          (this.selectorWidth / this.perPage) * this.innerOffset
      : currentSlide * this.widthItem;
  }
